...
1. **Етап Prune**: Використовує `turbo prune @mono/backend --docker` для створення мінімального робочого простору з лише необхідними залежностями
2. **Етап Installer**: Встановлює залежності з обрізаного робочого простору та збирає застосунок за допомогою `turbo build --filter=@mono/backend`
