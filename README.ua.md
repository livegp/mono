...
3. Створіть нову гілку для ваших змін (`git checkout -b feature/НоваФункція` або `bugfix/ВиправленняПомилки`).
4. Зробіть ваші зміни та закомітьте їх (`git commit -m 'Додано нову функціональність'`).
5. Відправте зміни у ваш форк (`git push origin feature/НоваФункція`).
